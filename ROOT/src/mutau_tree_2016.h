...
class mutau_tree2016 : public virtual base_tree {
   ...
   public:
    ...
    // Member functions
    mutau_tree2016(TTree* orig, TTree* itree, bool isMC, bool isEmbed, bool IsSignal, Int_t rec);
...
//////////////////////////////////////////////////////////////////
mutau_tree2016::mutau_tree2016(TTree* Original, TTree* itree, bool IsMC, bool IsEmbed, bool IsSignal, Int_t rec)
    : tree(itree),
      original(Original),
      ...
      isEmbed(IsEmbed),
      isSignal(IsSignal),
      tfes("2016Legacy", "DeepTau2017v2p1VSe", "ltau_skimmer/ROOT/data/", "mt", isEmbed),
      recoil(rec),
      era(2016) {}
...
void mutau_tree2016::do_skimming(TH1F* cutflow) {
    ...
    for (auto ievt = 0; ievt < nevt; ievt++) {
        ...
        if (isEmbed) {
            if ((IsoMu22 || IsoTkMu22 || IsoMu22eta2p1 || IsoTkMu22eta2p1) && mu.Pt() > 23) {
                cutflow->Fill(2., 1.);
            } else if (Cross_emb && mu.Pt() > 20 && mu.Pt() < 23 && fabs(tau.Eta()) < 2.1) {
                cutflow->Fill(2., 1.);
            } else {
                continue;
            }
            ...
            if ((IsoMu22 || IsoTkMu22 || IsoMu22eta2p1 || IsoTkMu22eta2p1) && mu.Pt() > 23) {
                cutflow->Fill(2., 1.);
            } else if ((Cross_v1 || Cross_v2) && mu.Pt() > 20 && mu.Pt() < 23 && fabs(tau.Eta()) < 2.1) {
                cutflow->Fill(2., 1.);
            } else {
                continue;
            }
...
void mutau_tree2016::do_met_corr_nom(Float_t sf, TLorentzVector tau, TLorentzVector* met) {
    *met = (*met) + tau - sf * tau;  // update input met}
}

void mutau_tree2016::do_recoil_corr(RecoilCorrector* recoilPFMetCorrector, TLorentzVector* met, int njets) {
...
//////////////////////////////////////////////////////////////////
TTree* mutau_tree2016::fill_tree(RecoilCorrector recoilPFMetCorrector, MEtSys metSys) {
