...
Float_t mutau_tree2016::get_tes_sf(Float_t decayMode) {
  if (decayMode == 0) {
    return  tes_dm0_sf;
  } else if (decayMode == 1) {
    return tes_dm1_sf;
...
Float_t mutau_tree2016::get_efake_sf(Float_t decayMode) {
  if (decayMode == 0) {
    return efake_dm0_sf;
  } else if (decayMode == 1) {
    return efake_dm1_sf;
...
Float_t mutau_tree2016::get_mfake_sf(Float_t decayMode) {
  if (decayMode == 0) {
    return mfake_dm0_sf;
  } else if (decayMode == 1) {
    return mfake_dm1_sf;
...
void mutau_tree2016::do_met_corr_nom(Float_t decayMode, energy_scale escale, TLorentzVector tau, TLorentzVector* met) {
  double sf(1.);
  if (escale == tes) {
...
TTree* mutau_tree2016::fill_tree(RecoilCorrector recoilPFMetCorrector) {
  ...
  for (auto& ievt : good_events) {
    ...
    mets = {
      &MET, &MET_UESUp, &MET_UESDown, &MET_JESUp, &MET_JESDown, &MET_Eta0to3Up, &MET_Eta0to3Down,
      &MET_Eta0to5Up, &MET_Eta0to5Down, &MET_Eta3to5Up, &MET_Eta3to5Down, &MET_EC2Up, &MET_EC2Down,
      &MET_RelBalUp, &MET_RelBalDown, &MET_RelSamUp, &MET_RelSamDown
    };
