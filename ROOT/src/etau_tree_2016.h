...
#include <utility>
#include <vector>
#include "../interface/et_2016_input_branches.h"
#include "./base_tree.h"
...
etau_tree2016::etau_tree2016(TTree* Original, TTree* itree, bool IsMC, bool IsEmbed, Int_t rec) : tree(itree),
                                                                                                  original(Original),
                                                                                                  in(new et_2016_input_branches(Original)),
                                                                                                  isMC(IsMC),
                                                                                                  isEmbed(IsEmbed),
                                                                                                  recoil(rec) {
  original->SetBranchStatus("double*", 0);
  original->SetBranchStatus("singleIso*", 0);
  original->SetBranchStatus("singleMu*", 0);
}
...
void etau_tree2016::do_skimming(TH1F* cutflow) {
  ...
  for (auto ievt = 0; ievt < nevt; ievt++) {
    ...
    auto Ele25 = in->singleE25eta2p1TightPass && in->eMatchesEle25eta2p1TightPath && in->eMatchesEle25TightFilter;
    ...
    if (!isEmbed || (in->eMatchesSingleE25Tight && in->singleE25eta2p1TightPass)) {
      cutflow->Fill(3., 1.);
    } else {
      continue;
    }
