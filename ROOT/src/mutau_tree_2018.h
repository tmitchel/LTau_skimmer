...
class mutau_tree2018 : public virtual base_tree {
   ...
   public:
    ...
    // Member functions
    mutau_tree2018(TTree* orig, TTree* itree, bool isMC, bool isEmbed, bool isSignal, Int_t rec);
...
//////////////////////////////////////////////////////////////////
mutau_tree2018::mutau_tree2018(TTree* Original, TTree* itree, bool IsMC, bool IsEmbed, bool IsSignal, Int_t rec)
    : tree(itree),
      original(Original),
...
void mutau_tree2018::do_skimming(TH1F* cutflow) {
    ...
    for (auto ievt = 0; ievt < nevt; ievt++) {
        ...
        if (in->bjetDeepCSVVeto20Medium_2018_DR0p5 < 1) {
            cutflow->Fill(12., 1.);
        } else {
            continue;
        }
...
//    tree->Branch("Phi", &in->Phi);
    ...
    tree->Branch("mMatchEmbeddedFilterMu20Tau27_2018", &in->mMatchEmbeddedFilterMu20Tau27_2018);
