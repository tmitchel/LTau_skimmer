   ...
   public:
    RecoilCorrector(TString fileName) {
        ...
        for (int i = 0; i <= nZPtBins; ++i) {
            ...
            if (i < nZPtBins) ZPtStr[i] = ZPtBinsH->GetXaxis()->GetBinLabel(i + 1);
        }
        ...
        InitMEtWeights(file, perpZStr, paralZStr, nZPtBins, ZPtBins, ZPtStr, nJetsBins, nJetsStr);
    ...
    void Correct(float MetPx, float MetPy, float genVPx, float genVPy, float visVPx, float visVPy, int njets, float& MetCorrPx, float& MetCorrPy) {
        float Zpt = TMath::Sqrt(genVPx * genVPx + genVPy * genVPy);
        float U1 = 0;
        float U2 = 0;
        float metU1 = 0;
        float metU2 = 0;
        CalculateU1U2FromMet(MetPx, MetPy, genVPx, genVPy, visVPx, visVPy, U1, U2, metU1, metU2);
        if (Zpt > 1000) Zpt = 999;

        if (njets >= _nJetsBins) njets = _nJetsBins - 1;
        ...
        if (U1 > _range * _xminMetZParal[ZptBin][njets] && U1 < _range * _xmaxMetZParal[ZptBin][njets]) {
            ...
            float U1reco = rescale(U1, _meanMetZParalData[ZptBin][njets], _meanMetZParalMC[ZptBin][njets], _rmsMetZParalData[ZptBin][njets],
        ...
        if (U2 > _range * _xminMetZPerp[ZptBin][njets] && U2 < _range * _xmaxMetZPerp[ZptBin][njets]) {
            ...
            float U2reco = rescale(U2, _meanMetZPerpData[ZptBin][njets], _meanMetZPerpMC[ZptBin][njets], _rmsMetZPerpData[ZptBin][njets],
    ...
    void CorrectByMeanResolution(float MetPx, float MetPy, float genVPx, float genVPy, float visVPx, float visVPy, int njets, float& MetCorrPx,
                                 float& MetCorrPy) {
        float Zpt = TMath::Sqrt(genVPx * genVPx + genVPy * genVPy);
   ...
   private:
    int binNumber(float x, const std::vector<float> bins) const {
        for (size_t iB = 0; iB < bins.size(); ++iB)
            if (x >= bins[iB] && x < bins[iB + 1]) return iB;
        return 0;
    }
    ...
    void InitMEtWeights(TFile* _file, TString _perpZStr, TString _paralZStr, int nZPtBins, float* ZPtBins, TString* _ZPtStr, int nJetsBins,
                        TString* _nJetsStr) {
        ...
        for (int idx = 0; idx < nZPtBins + 1; ++idx) newZPtBins.push_back(ZPtBins[idx]);
        for (int idx = 0; idx < nZPtBins; ++idx) newZPtStr.push_back(std::string(_ZPtStr[idx]));

        for (int idx = 0; idx < nJetsBins; ++idx) newNJetsStr.push_back(std::string(_nJetsStr[idx]));

        InitMEtWeights(_file, newZPtBins, newPerpZStr, newParalZStr, newZPtStr, newNJetsStr);
    }

    void InitMEtWeights(TFile* _fileMet, const std::vector<float>& ZPtBins, const std::string _perpZStr, const std::string _paralZStr,
                        const std::vector<std::string>& _ZPtStr, const std::vector<std::string>& _nJetsStr) {
        ...
        for (int ZPtBin = 0; ZPtBin < _nZPtBins; ++ZPtBin) {
            for (int jetBin = 0; jetBin < _nJetsBins; ++jetBin) {
                ...
                if (_metZParalData[ZPtBin][jetBin] == NULL) {
                    std::cout << "Function with name " << binStrParalData << " is not found in file " << _fileName << "... quitting program..."
                              << std::endl;
                    exit(-1);
                }
                if (_metZPerpData[ZPtBin][jetBin] == NULL) {
                    std::cout << "Function with name " << binStrPerpData << " is not found in file " << _fileName << "... quitting program..."
                              << std::endl;
                    exit(-1);
                }

                if (_metZParalMC[ZPtBin][jetBin] == NULL) {
                    std::cout << "Function with name " << binStrParalMC << " is not found in file " << _fileName << "... quitting program..."
                              << std::endl;
                    exit(-1);
                }
                if (_metZPerpMC[ZPtBin][jetBin] == NULL) {
                    std::cout << "Function with name " << binStrPerpMC << " is not found in file " << _fileName << "... quitting program..."
                              << std::endl;
                    exit(-1);
                }
                ...
                _meanMetZParalData[ZPtBin][jetBin] =
                    _metZParalData[ZPtBin][jetBin]->Mean(_xminMetZParalData[ZPtBin][jetBin], _xmaxMetZParalData[ZPtBin][jetBin]);
                _rmsMetZParalData[ZPtBin][jetBin] = TMath::Sqrt(
                    _metZParalData[ZPtBin][jetBin]->CentralMoment(2, _xminMetZParalData[ZPtBin][jetBin], _xmaxMetZParalData[ZPtBin][jetBin]));
                _meanMetZPerpData[ZPtBin][jetBin] = 0;
                _rmsMetZPerpData[ZPtBin][jetBin] = TMath::Sqrt(
                    _metZPerpData[ZPtBin][jetBin]->CentralMoment(2, _xminMetZPerpData[ZPtBin][jetBin], _xmaxMetZPerpData[ZPtBin][jetBin]));

                _meanMetZParalMC[ZPtBin][jetBin] =
                    _metZParalMC[ZPtBin][jetBin]->Mean(_xminMetZParalMC[ZPtBin][jetBin], _xmaxMetZParalMC[ZPtBin][jetBin]);
                _rmsMetZParalMC[ZPtBin][jetBin] =
                    TMath::Sqrt(_metZParalMC[ZPtBin][jetBin]->CentralMoment(2, _xminMetZParalMC[ZPtBin][jetBin], _xmaxMetZParalMC[ZPtBin][jetBin]));
                _meanMetZPerpMC[ZPtBin][jetBin] = 0;
                _rmsMetZPerpMC[ZPtBin][jetBin] =
                    TMath::Sqrt(_metZPerpMC[ZPtBin][jetBin]->CentralMoment(2, _xminMetZPerpMC[ZPtBin][jetBin], _xmaxMetZPerpMC[ZPtBin][jetBin]));
            }
        }
    }
    void CalculateU1U2FromMet(float metPx, float metPy, float genZPx, float genZPy, float diLepPx, float diLepPy, float& U1, float& U2, float& metU1,
                              float& metU2) {
    ...
    void CalculateMetFromU1U2(float U1, float U2, float genZPx, float genZPy, float diLepPx, float diLepPy, float& metPx, float& metPy) {
    ...
    void U1U2CorrectionsByWidth(float& U1, float& U2, int ZptBin, int njets) {
        if (njets >= _nJetsBins) njets = _nJetsBins - 1;
    ...
    float rescale(float x, float meanData, float meanMC, float resolutionData, float resolutionMC) {
