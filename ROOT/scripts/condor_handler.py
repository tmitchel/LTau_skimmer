...
def write_bash_script(command, output_sample_name, dag_dir, use_input=''):
    ...
    bashScript += command + ' {} $value'.format(use_input)
    bashScript += '\n rm input_file.root'
    bashScript += '\n'
...
def submit_command(command, jobName, input_sample_dir, output_sample_name=None, dryrun=False, extra_inputs=None, bigboi=False, use_input=''):
    ...
    # create bash script
    bash_name = write_bash_script(command, output_sample_name, dag_dir, use_input)
