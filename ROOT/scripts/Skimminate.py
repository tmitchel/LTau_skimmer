...
def parse_command_line(argv):
    parser = argparse.ArgumentParser(description="Run the desired analyzer on FSA n-tuples")
...
def main(argv=None):
    ...
    if sample_name == '' :
        ...
        sample_dir = '/nfs_scratch/%s/%s/%s' % (pwd.getpwuid(os.getuid())[0], jobName, args.samplename)

    # create submit dir
    submit_dir = '%s/submit' % (sample_dir)
    if os.path.exists(submit_dir):
        print('Submission directory exists for %s %s.' % (jobName, args.samplename))
